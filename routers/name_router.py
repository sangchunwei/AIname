...
async def get_names(names:NameIn,user_id:int=Depends(auth_handler.auth_access_dependency),session:AsyncSession=Depends(get_session)) :
    #user_id用户创建表时用的,当时创的时候用id指定表明,现在查应该用相同的名字才可以
    await ensure_quota(session, user_id, "name_generation")
    result=await generate_naming(names,user_id)#返回模型的输出数据
    await consume_quota(session, user_id, "name_generation")
...
async def take_names_first_time(names:NameIn,user_id:int=Depends(auth_handler.auth_access_dependency),session:AsyncSession=Depends(get_session)) :
    #user_id用户创建表时用的,当时创的时候用id指定表明,现在查应该用相同的名字才可以
    await ensure_quota(session, user_id, "name_generation")
...
async def take_names_feedback(data:FeedbackIn,user_id:int=Depends(auth_handler.auth_access_dependency),session:AsyncSession=Depends(get_session)) :
    """带有 Thread_ID 的多轮微调"""
    await ensure_quota(session, user_id, "name_generation")
