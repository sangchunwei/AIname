...
> 当前文档版本：`1.0.4`（2026-07-02）。本版本在 `1.0.3` 的开放平台、版本更新、社区互动和 RAG 队列基础上，新增支付宝沙箱 H5 支付、统一支付交易表、支付结果页、Docker/Nginx 部署配置，并将 VIP、专家服务和钱包充值从本地模拟支付升级为可联调的沙箱支付链路。
...
1.0.4 新增支付宝沙箱充值和统一支付交易记录，VIP、专家订单和钱包充值可以打开支付宝沙箱 H5 收银台；提现申请统一为支付宝账号，后台审核后仍按本地账本流程处理。当前支付宝能力用于沙箱联调，提现和佣金仍不具备正式代付能力。

### 支付宝沙箱支付

1.0.4 新增统一支付模块：

- VIP 订单、专家订单和钱包充值均可创建支付宝沙箱 H5 收银台支付链接。
- 新增 `/payments/alipay/vip/{order_id}/page-pay`、`/payments/alipay/expert/{order_id}/page-pay`、`/payments/alipay/recharge/{order_id}/page-pay`。
- 新增支付宝异步通知、同步查询、回跳页和本地开发确认接口。
- 新增前端支付结果页，支持按 `out_trade_no` 刷新支付状态并回到钱包或用户中心。
- 本地模拟支付接口已停用，避免测试逻辑误入生产链路；余额支付仍保留用于钱包余额消费。
...
- 缓存与视觉任务队列：Redis。
- RAG 后台队列：RabbitMQ。
- 支付联调：支付宝沙箱 H5 收银台、RSA2 签名、统一支付交易表。
- 部署：Dockerfile、docker-compose、Nginx 反向代理示例。
- 大语言模型：DeepSeek。
...
> 注意：`ainame/manifest.json`、`ainame/utils/app-version.js`、`.env.example` 和后端默认版本配置已同步为 `1.0.4/104`。
...
- `b8f4c2d91a7e`：支付宝沙箱统一支付交易表 `payment_transaction`。
...
```env
AINAME_ALIPAY_APP_ID=你的支付宝沙箱 App ID
AINAME_ALIPAY_PRIVATE_KEY=你的应用私钥
AINAME_ALIPAY_PUBLIC_KEY=支付宝公钥
AINAME_ALIPAY_NOTIFY_URL=http://127.0.0.1:8000/payments/alipay/notify
AINAME_ALIPAY_RETURN_URL=http://127.0.0.1:8000/payments/alipay/return
```
...
- `AINAME_MOCK_PAYMENT_ENABLED` 不得在生产环境启用。
- 支付宝私钥、公钥和 App ID 只应写入本地 `.env` 或部署平台密钥管理，不要提交到 Git。
